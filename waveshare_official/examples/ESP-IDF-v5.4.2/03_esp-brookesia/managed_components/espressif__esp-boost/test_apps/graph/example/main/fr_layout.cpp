...
BOOST_AUTO_TEST_CASE(fr_layout)
{
    TEST_ASSERT(test_main(0, nullptr) == 0);
    /* esp32: test fails to compile due to its dependency on Boost::timer, which is not included in the current port of Boost */
    std::cout << std::endl;
    std::cout << "!!! THIS TEST IS SKIPPED, AS IT RELIES ON Boost::timer. !!!" << std::endl;
    std::cout << std::endl;
}
