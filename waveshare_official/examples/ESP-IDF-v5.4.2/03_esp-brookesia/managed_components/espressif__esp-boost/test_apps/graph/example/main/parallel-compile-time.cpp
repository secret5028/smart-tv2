...
static int test_main(int argc, const char** argv)
{
...

    {
        ...
        = argc >= 3 ? argv[2] : "makefile-dependencies.dat";
        auto name_in_ptr = open_input_stream(embedded_files, dep_name_in);
        std::istream& name_in = *name_in_ptr;
        const char* compile_cost_in_name
            = argc >= 4 ? argv[3] : "target-compile-costs.dat";
        auto compile_cost_in_ptr = open_input_stream(embedded_files, compile_cost_in_name);
...
BOOST_AUTO_TEST_CASE(parallel-compile-time)
{
