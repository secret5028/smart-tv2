...
static int test_main(int argc, const char** argv)
{
    ...
    const char* datafile_name
        = argc >= 2 ? argv[1] : "boost_web.dat";
    auto datafile_ptr = open_input_stream(embedded_files, datafile_name);
    std::istream& datafile = *datafile_ptr;
    if (!datafile)
    {
...
BOOST_AUTO_TEST_CASE(boost_web_graph)
{
