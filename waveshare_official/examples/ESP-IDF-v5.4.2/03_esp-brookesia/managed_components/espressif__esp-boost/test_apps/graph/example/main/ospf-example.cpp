...
static int test_main(int argc, const char** argv)
{
            ...
            get_property(g_dot, graph_color)));
    {
        ...
        const char* infile_name
            = argc >= 2 ? argv[1] : "figs/ospf-graph.dot";
        auto infile_ptr = open_input_stream(embedded_files, infile_name);
        read_graphviz(*infile_ptr, g_dot, dp);
    }
    ...
    get_property(g_dot, graph_color) = "grey";
    {
        ...
        write_graphviz_dp(std::cout, g_dot, dp);
    }
    ...
    for (boost::tie(vi, vi_end) = vertices(g_dot); vi != vi_end; ++vi)
    {
        if (parent[*vi] != *vi)
        {
            std::cout << get(vertex_name, g_dot, *vi) << "    ";
            vertex_descriptor v = *vi, child;
            int path_cost = 0;
            ...
            std::cout << get(vertex_name, g_dot, child) << "     ";
...
BOOST_AUTO_TEST_CASE(ospf-example)
{
