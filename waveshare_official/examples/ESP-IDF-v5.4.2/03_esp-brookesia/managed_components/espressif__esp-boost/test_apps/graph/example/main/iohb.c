...
int readHB_info(FILE* in_file, int* M, int* N, int* nz, char** Type,
                                                      int* Nrhs)
{
/****************************************************************************/
/*  The readHB_info function opens and reads the header information from    */
...
/*  Fourth line:  */
    ...
    if ( sscanf(line, "%16c",Ptrfmt) != 1)
        IOHBTerminate("iohb.c: Invalid format info1, line 4 of Harwell-Boeing file.\n");
    if ( sscanf(line, "%*16c%16c",Indfmt) != 1)
        IOHBTerminate("iohb.c: Invalid format info2, line 4 of Harwell-Boeing file.\n");
    if ( sscanf(line, "%*16c%*16c%20c",Valfmt) != 1)
        IOHBTerminate("iohb.c: Invalid format info3, line 4 of Harwell-Boeing file.\n");
...
int readHB_mat_double(FILE* in_file, int colptr[], int rowind[],
                                                                 double val[])
{
/****************************************************************************/
/*  This function opens and reads the specified file, interpreting its      */
...
int readHB_newmat_double(FILE* in_file, int* M, int* N, int* nonzeros,
                         int** colptr, int** rowind, double** val)
{
        int Nrhs;
        char *Type;
...
*/
        ...
        return readHB_mat_double(in_file, *colptr, *rowind, *val);

}

int readHB_aux_double(FILE* in_file, const char AuxType, double b[])
{
/****************************************************************************/
...
int readHB_newaux_double(FILE* in_file, const char AuxType, double** b)
{
        ...
        if ( Nrhs <= 0 ) {
          ...
          if ( Type[0] == 'C' ) {
            ...
            *b = (double *)malloc(M*Nrhs*sizeof(double)*2);
            ...
            return readHB_aux_double(in_file, AuxType, *b);
          } else {
