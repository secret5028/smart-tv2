...
static movable2 move_return_function2(int i) {
  return movable2(i);
}
...
static int test_main()
{
...

  {
    ...
    /* esp32: check if the future is ready before waiting to avoid deadlock */
    if (!f.is_ready()) {
      f.wait();
    }
  }
...
BOOST_AUTO_TEST_CASE(sync/futures, make_ready_future_pass)
{
    common_init();
    std::thread([&]() {
