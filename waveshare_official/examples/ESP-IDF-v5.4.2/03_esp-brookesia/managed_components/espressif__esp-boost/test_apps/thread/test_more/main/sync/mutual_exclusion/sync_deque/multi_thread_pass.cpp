...
static void test_concurrent_push_back_and_pull_front_on_empty_queue()
{
...
static void test_concurrent_push_back_and_wait_pull_front_on_empty_queue()
{
  boost::sync_deque<int> q;
...
static void test_concurrent_wait_pull_front_and_close_on_empty_queue()
{
  boost::sync_deque<int> q;
  const unsigned int n = 3;
  boost::barrier go(n);
...
static void test_concurrent_push_back_on_empty_queue()
{
  boost::sync_deque<int> q;
  const unsigned int n = 3;
  boost::barrier go(n);
...
static void test_concurrent_pull_front_on_queue()
{
  boost::sync_deque<int> q;
  const unsigned int n = 3;
  boost::barrier go(n);
...
BOOST_AUTO_TEST_CASE(sync/mutual_exclusion/sync_deque, multi_thread_pass)
{
    common_init();
    std::thread([&]() {
