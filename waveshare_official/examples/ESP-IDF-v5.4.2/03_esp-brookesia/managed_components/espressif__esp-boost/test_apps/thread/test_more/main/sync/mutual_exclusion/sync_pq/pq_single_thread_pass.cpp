...
static void test_nonblocking_pull()
{
  sync_pq pq;
  steady_clock::time_point start = steady_clock::now();
  int val;
...
static void test_pull_for_when_not_empty()
{
  sync_pq pq;
  pq.push(1);
  steady_clock::time_point start = steady_clock::now();
...
static void test_pull_until_when_not_empty()
{
  sync_pq pq;
  pq.push(1);
  steady_clock::time_point start = steady_clock::now();
...
BOOST_AUTO_TEST_CASE(sync/mutual_exclusion/sync_pq, pq_single_thread_pass)
{
    common_init();
    std::thread([&]() {
