...
static boost::csbl::unique_ptr<int> f3_0()
{
  boost::this_thread::sleep_for(ms(200));
  boost::csbl::unique_ptr<int> r( (new int(3)));
  return boost::move(r);
}
static MoveOnly f3_1()
{
  boost::this_thread::sleep_for(ms(200));
  MoveOnly r;
  return boost::move(r);
...
static boost::csbl::unique_ptr<int> f3(int i)
{
  boost::this_thread::sleep_for(ms(200));
  return boost::csbl::unique_ptr<int>(new int(i));
}

static boost::csbl::unique_ptr<int> f4(
...
BOOST_AUTO_TEST_CASE(sync/futures/async, async_executor_pass)
{
    common_init();
    std::thread([&]() {
