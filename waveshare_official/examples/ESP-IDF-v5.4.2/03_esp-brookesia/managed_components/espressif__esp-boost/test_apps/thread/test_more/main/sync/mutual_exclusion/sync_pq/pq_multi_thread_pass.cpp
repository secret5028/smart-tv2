...
static void call_push(sync_pq* q, boost::barrier* go, int val)
{
    go->wait();
    q->push(val);
}

static void test_pull(const int n)
{
    sync_pq pq;
    BOOST_TEST(pq.empty());
    for(int i  = 0; i < n; i++)
...
static void test_push(const int n)
{
    sync_pq pq;
    BOOST_TEST(pq.empty());
...
static void test_both(const int n)
{
    sync_pq pq;
    BOOST_TEST(pq.empty());
...
static void push_range(sync_pq* q, const int begin, const int end)
{
    for(int i = begin; i < end; i++)
        q->push(i);
}

static void atomic_pull(sync_pq* q, boost::atomic<int>* sum)
{
    while(1)
    {
        try{
 ...
 */
static void compute_sum(const int n)
{
...
static void move_between_queues(sync_pq* q1, sync_pq* q2)
{
    while(1){
        try{
 ...
 * problem.
 */
static void sum_with_moving(const int n)
{
    const int limit = 1000;
    sync_pq pq1;
    sync_pq pq2;
...
/* esp32: replace 60 with 10 to avoid error `E (11768) pthread: Failed to create task!` */
...
BOOST_AUTO_TEST_CASE(sync/mutual_exclusion/sync_pq, pq_multi_thread_pass)
{
    common_init();
    std::thread([&]() {
