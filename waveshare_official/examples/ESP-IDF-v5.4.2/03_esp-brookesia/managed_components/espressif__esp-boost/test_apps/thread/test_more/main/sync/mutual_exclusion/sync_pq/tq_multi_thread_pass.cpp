...
static void call_push(sync_tq* q, const steady_clock::time_point start)
{
...
static void call_pull(sync_tq* q, const steady_clock::time_point start)
{
...
static void call_pull_until(sync_tq* q, const steady_clock::time_point start)
{
    // pull elements off of the queue (earliest element first)
    for (int i = cnt - 1; i >= 0; --i)
    {
...
static void call_pull_for(sync_tq* q, const steady_clock::time_point start)
{
    // pull elements off of the queue (earliest element first)
    for (int i = cnt - 1; i >= 0; --i)
    {
...
static void test_push_while_pull()
{
    sync_tq tq;
    BOOST_TEST(tq.empty());
...
static void test_push_while_pull_until()
{
    sync_tq tq;
    BOOST_TEST(tq.empty());
    boost::thread_group tg;
...
static void test_push_while_pull_for()
{
    sync_tq tq;
    BOOST_TEST(tq.empty());
    boost::thread_group tg;
...
BOOST_AUTO_TEST_CASE(sync/mutual_exclusion/sync_pq, tq_multi_thread_pass)
{
    common_init();
    std::thread([&]() {
